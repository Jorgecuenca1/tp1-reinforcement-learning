...
Actualiza Q(s,a) usando max_a' Q(s',a') independientemente de la accion tomada.
Converge a la politica optima bajo condiciones estandar.
"""
import numpy as np
...
def train(env_id, env_kwargs=None, num_episodes=5000, gamma=0.99,
    ...
    for episode in range(num_episodes):
        ...
        while not done and not truncated:
            ...
            max_q_next = max(Q[(next_state, a)] for a in range(n_actions)) if not done else 0.0
...
for ep in range(NUM_EPISODES):
    ...
    while not done and not truncated:
        if np.random.random() < epsilon:
            ...
            action = int(np.argmax([Q[(state, a)] for a in range(n_actions)]))
...
plt.title("Q-Learning - {problem_name}", fontsize=15, fontweight="bold")
